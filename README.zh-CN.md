...
> **JDK 基线**：21
> **项目状态**：活跃维护（3.0.x 线）。快照制品通过阿里云 Maven 仓库分发。
...
- **不是 OkHttp 的分支**。基于官方 `okhttp-jvm` 构件（本线为 5.4.0）扩展。
...
- POM 通过 `maven-enforcer-plugin` 强制 Maven 与 JDK 21 基线。
